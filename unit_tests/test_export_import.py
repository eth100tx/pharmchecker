...
class ExportImportTester:
    ...
    def test_export_validated(self, tag):
        """Test exporting a validated dataset"""
        try:
            print(f"\n=== Testing Export: Validated '{tag}' ===")
            
            # Get dataset ID
            datasets = self.client.get_datasets()
            dataset_id = None
            for d in datasets:
                if d.get('kind') == 'validated' and d.get('tag') == tag:
                    dataset_id = d.get('id')
                    break
            
            if not dataset_id:
                self.log_result(f"export_validated_{tag}", False, f"Dataset '{tag}' not found")
                return None
            
            # Get validated data using the table endpoint
            import requests
            url = f"{self.client.supabase_client.url}/rest/v1/validated_overrides"
            headers = self.client.supabase_client.headers
            
            response = requests.get(url, 
                                  headers=headers,
                                  params={'dataset_id': f'eq.{dataset_id}', 'limit': '9999'},
                                  timeout=30)
            
            if response.status_code != 200:
                self.log_result(f"export_validated_{tag}", False, f"API error: {response.status_code} {response.text}")
                return None
            
            data = response.json()
            
            if not data:
                self.log_result(f"export_validated_{tag}", False, "No data returned from API")
                return None
            
            # Convert to DataFrame and CSV (clean format for import)
            df = pd.DataFrame(data)
            
            # Export only essential fields (exclude database internals like id, dataset_id)
            essential_cols = ['pharmacy_name', 'state_code', 'license_number', 'license_status', 
                            'license_name', 'address', 'city', 'state', 'zip', 'issue_date', 
                            'expiration_date', 'result_status', 'override_type', 'reason', 
                            'validated_by', 'validated_at']
            export_cols = [col for col in essential_cols if col in df.columns]
            
            export_df = df[export_cols].copy()
            
            # Clean up date fields that might cause import issues
            date_fields = ['issue_date', 'expiration_date', 'validated_at']
            for field in date_fields:
                if field in export_df.columns:
                    # Ensure dates are in proper format
                    export_df[field] = pd.to_datetime(export_df[field], errors='coerce').dt.strftime('%Y-%m-%d %H:%M:%S')
            
            csv_data = export_df.to_csv(index=False)
            
            self.log_result(f"export_validated_{tag}", True, f"Exported {len(export_df)} records ({len(export_cols)} fields), {len(csv_data)} bytes")
            
            # Return both DataFrame and CSV for testing
            return {'dataframe': df, 'csv': csv_data, 'original_count': len(df)}
            
        except Exception as e:
            self.log_result(f"export_validated_{tag}", False, f"Exception: {e}")
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            return None
    
    def test_import_validated(self, csv_data, new_tag):
        """Test importing validated data from CSV"""
        try:
            print(f"\n=== Testing Import: Validated '{new_tag}' ===")
            ...
            if result.returncode != 0:
                self.log_result(f"import_validated_{new_tag}", False, 
                              f"Import failed with code {result.returncode}", 
                              {'stdout': result.stdout, 'stderr': result.stderr})
                return False
            ...
            imported_dataset = None
            for d in datasets:
                if d.get('kind') == 'validated' and d.get('tag') == new_tag:
                    imported_dataset = d
                    break
            
            if not imported_dataset:
                self.log_result(f"import_validated_{new_tag}", False, "Dataset not found after import")
                return False
            
            # Get imported data count via API
            import requests
            url = f"{self.client.supabase_client.url}/rest/v1/validated_overrides"
            headers = self.client.supabase_client.headers
            ...
            response = requests.get(url, 
                                  headers=headers,
                                  params={'dataset_id': f'eq.{imported_dataset["id"]}', 'limit': '9999'},
                                  timeout=30)
            ...
            self.log_result(f"import_validated_{new_tag}", True, f"Imported {imported_count} records")
            ...
            self.log_result(f"import_validated_{new_tag}", False, f"Exception: {e}")
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            return False
    
    def test_round_trip_validated(self, original_tag):
        """Test complete export-import round trip for validated data"""
        try:
            print(f"\n=== Testing Round Trip: Validated '{original_tag}' ===")
            
            # Step 1: Export
            export_result = self.test_export_validated(original_tag)
            if not export_result:
                return False
            
            # Step 2: Import with new tag
            import_tag = f"{original_tag}_imported_{int(datetime.now().timestamp())}"
            import_result = self.test_import_validated(export_result['csv'], import_tag)
            if not import_result:
                return False
            ...
            if original_count != imported_count:
                self.log_result(f"roundtrip_validated_{original_tag}", False, 
                              f"Record count mismatch: {original_count} vs {imported_count}")
                return False
            ...
            # Compare key columns (ignore IDs and timestamps)
            compare_cols = ['pharmacy_name', 'state_code', 'license_number', 'override_type']
            available_cols = [col for col in compare_cols if col in original_df.columns and col in imported_df.columns]
            ...
            self.log_result(f"roundtrip_validated_{original_tag}", data_matches, 
                          f"Round trip {'successful' if data_matches else 'failed'}: {original_count} records")
            
            return data_matches
            
        except Exception as e:
            self.log_result(f"roundtrip_validated_{original_tag}", False, f"Exception: {e}")
            import traceback
            print(f"Traceback: {traceback.format_exc()}")
            return False
    ...
    def run_comprehensive_tests(self):
        ...
        try:
            ...
            validated_datasets = [d['tag'] for d in datasets if d.get('kind') == 'validated']
            
            print(f"Available datasets:")
            print(f"  Pharmacies: {pharmacy_datasets}")
            print(f"  States: {states_datasets}")
            print(f"  Validated: {validated_datasets}")
            ...
            # Test validated round trips
            for tag in validated_datasets[:2]:  # Test first 2 validated datasets
                self.test_round_trip_validated(tag)
